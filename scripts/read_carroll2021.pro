...
;   
; CALLING SEQUENCE:
;   read_sed_phot, file, [, /MASK, /DERED, /FORCED_PHOT, /MIN_ERR ]
;
; INPUTS:
...
for f = 0,nfiles-1 do begin
    ...
    if keyword_set(dered) then $
        for i = 0,nbands-1 do $
            ;; ***** NOTE ***** if using SDSS photometry that has NOT been deredenned, comment out the line below
    ...
    iaccept = where(iizrang,ct,complement=irem,ncomplement=nrem)
    if (ct eq 0) then continue
    ...
    iaccept = where(iiclean eq 1,ct,complement=irem,ncomplement=nrem)
    if (ct eq 0) then continue
    if (nrem gt 0) then obs[irem].iiaccept = 0
    ...
    iaccept = where(iifourw,ct,complement=irem,ncomplement=nrem)
    if (ct eq 0) then continue
    if (nrem gt 0) then obs[irem].iiaccept = 0
    ...
    if keyword_set(mask) then begin
        ...
        iinomsk = ~in_allwise
        iaccept = where(iinomsk,ct,complement=irem,ncomplement=nrem)		 ;; sources not in reject mask
        if (ct eq 0.) then continue
        if (nrem gt 0) then obs[irem].iiaccept = 0
    endif
    ...
    if (n_elements(uniq(obs.objid,sort(obs.objid))) ne n_elements(obs)) then begin
        ...
        iinodup[ikeep] = 1
        iaccept = where(iinodup,ct,complement=irem,ncomplement=nrem)     ;; remove duplicates
        if (ct eq 0.) then continue
        if (nrem gt 0) then obs[irem].iiaccept = 0
    endif
